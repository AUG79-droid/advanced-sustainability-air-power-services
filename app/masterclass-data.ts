...
const chapterTitles = [
  t("Primero: entender qué cambia y por qué", "First: understand what changes and why"),
  t("Después: medir sin engañarnos", "Then: measure without misleading ourselves"),
  t("Por último: probar, decidir y volver a comprobar", "Finally: test, decide and check again"),
];
  ...
  return [
    {
      ...
      kicker: t("EXPLICACIÓN EN LENGUAJE CLARO", "PLAIN-LANGUAGE EXPLANATION"),
      paragraphs: [
        t(`Para estudiar ${p.name.es} empezamos con una pregunta sencilla: «¿qué trabajo tiene que seguir haciéndose correctamente?». Después miramos todo lo que interviene: ${p.system.es}. Esto evita mejorar una cifra a costa de la seguridad, de la disponibilidad o de otra parte del proceso.`, `To study ${p.name.en}, start with a simple question: ‘Which job must continue to be performed correctly?’ Then look at everything involved: ${p.system.en}. This prevents one figure being improved at the expense of safety, availability or another part of the process.`),
        t(`El principal peligro es que ${p.problem.es}. Para evitarlo, seguimos la secuencia como haríamos al investigar un problema técnico: qué cambió, qué efecto inmediato produjo, qué resultado observamos y qué otra explicación podría existir. Una diferencia entre dos periodos no demuestra por sí sola que la medida sea la causa.`, `The main danger is that ${p.problem.en}. To avoid it, follow the same sequence used to investigate a technical problem: what changed, which immediate effect it produced, which result was observed and what other explanation may exist. A difference between two periods does not prove by itself that the measure caused it.`),
        t(`Conviene revisar cuatro grupos de factores: ${p.mechanisms.map((x) => x.es).join("; ")}. No hace falta dominar medio ambiente para empezar: basta con describir cada paso con personas de Operaciones, Ingeniería, Mantenimiento, Prevención, Compras y Medio Ambiente, y anotar qué dato confirmaría o descartaría la explicación.`, `Review four groups of factors: ${p.mechanisms.map((x) => x.en).join("; ")}. Environmental expertise is not required to begin: describe each step with people from Operations, Engineering, Maintenance, Safety, Procurement and Environment, and record which data would confirm or reject the explanation.`),
      ],
      keyPoints: p.mechanisms,
      prompt: t(`Piensa en una tarea de tu área relacionada con ${p.name.es}. ¿Qué cambia primero, qué resultado esperas y qué podría empeorar sin darte cuenta?`, `Think of a task in your area related to ${p.name.en}. What changes first, which result do you expect and what could worsen unnoticed?`),
    },
    {
      title: chapterTitles[1],
      kicker: t("MÉTODO Y DATOS", "METHOD AND DATA"),
      paragraphs: [
        t(`Para poder revisar la conclusión necesitamos ${p.evidence.es}. Un dato útil siempre responde a seis preguntas: ¿qué se midió?, ¿en qué unidad?, ¿durante qué periodo?, ¿para qué tareas o equipos?, ¿de qué sistema procede? y ¿quién puede explicar el cálculo? Si falta una respuesta, el dato no se elimina, pero su limitación se hace visible.`, `To review the conclusion we need ${p.evidence.en}. Useful data answers six questions: what was measured, in which unit, during which period, for which tasks or equipment, from which system and who can explain the calculation? If an answer is missing, the data are not discarded, but the limitation is made visible.`),
        t(`El indicador principal es ${p.metric.es}. Aquí aparecen dos ideas fáciles de confundir. El total muestra la carga completa —por ejemplo, litros consumidos al año—. La intensidad divide ese total por una actividad —por ejemplo, litros por misión comparable—. Necesitamos ambos: el primero muestra el efecto global y el segundo ayuda a saber si cada tarea se hizo mejor.`, `The main indicator is ${p.metric.en}. Two easily confused ideas appear here. The total shows the complete burden—for example, litres used per year. Intensity divides that total by activity—for example, litres per comparable mission. Both are needed: the first shows the overall effect and the second helps determine whether each task was performed better.`),
        t(`Los datos nunca son perfectos. En este tema, las dudas principales proceden de ${p.uncertainty.es}. En vez de inventar exactitud, usamos un rango y preguntamos: «si este supuesto cambia, ¿seguiríamos eligiendo lo mismo?». Si la respuesta es no, ese supuesto necesita un ensayo o una medición mejor antes de aprobar a gran escala.`, `Data are never perfect. In this topic, the main doubts come from ${p.uncertainty.en}. Instead of inventing precision, use a range and ask: ‘If this assumption changes, would we still choose the same option?’ If the answer is no, that assumption needs a trial or better measurement before full-scale approval.`),
      ],
      keyPoints: [t("Qué se midió y en qué unidad", "What was measured and in which unit"), t("Total y resultado por tarea", "Total and result per task"), t("Comparaciones equivalentes", "Equivalent comparisons"), t("Dudas y datos ausentes visibles", "Visible doubts and missing data")],
      prompt: t(`Elige un indicador de tu trabajo. ¿Puedes explicar de dónde sale, qué incluye y qué conclusión no permite obtener?`, `Choose one indicator from your work. Can you explain where it comes from, what it includes and which conclusion it does not support?`),
    },
    {
      title: chapterTitles[2],
      kicker: t("DECISIÓN Y CONTROL", "DECISION AND CONTROL"),
      paragraphs: [
        t(`La decisión difícil aparece porque ${p.tradeoff.es}. El orden importa: primero se comprueba seguridad, cumplimiento, autoridad técnica y capacidad mínima. Una alternativa que no supera una de esas condiciones no se compensa con una buena cifra ambiental. Solo después se comparan consumo, residuos, emisiones, coste u otros resultados.`, `The difficult decision appears because ${p.tradeoff.en}. Order matters: safety, compliance, technical authority and minimum capability are checked first. An option failing one of those conditions is not compensated by a good environmental figure. Consumption, waste, emissions, cost and other results are compared only afterwards.`),
        t(`La acción recomendada es ${p.intervention.es}. No se implanta inmediatamente en toda la flota o en todo el centro. Se empieza con un piloto que indique qué equipos participan, cuánto dura, qué situación sirve de referencia, quién puede detenerlo y qué resultado mínimo se exige. Así, un fallo produce aprendizaje controlado y no una consecuencia extendida.`, `The recommended action is ${p.intervention.en}. It is not immediately implemented across the entire fleet or site. Begin with a pilot stating which equipment participates, duration, reference situation, who can stop it and the minimum required result. A failure then produces controlled learning rather than a widespread consequence.`),
        t(`Después se verifica mediante ${p.verification.es}. «Verificar» significa volver a mirar los datos tras el cambio y comprobar también lo que podía empeorar. Si la mejora solo funciona en una configuración, estación o tipo de tarea, se conserva esa limitación. Una buena lección termina modificando la instrucción, la formación o el criterio de decisión.`, `Verification then uses ${p.verification.en}. ‘Verify’ means reviewing data after the change and also checking what could have worsened. If the improvement works only in one configuration, season or task type, that limitation remains. A good lesson ends by changing the instruction, training or decision rule.`),
      ],
      keyPoints: [t("Seguridad y misión primero", "Safety and mission first"), t("Piloto pequeño y controlado", "Small controlled pilot"), t("Resultado y efecto adverso", "Outcome and adverse effect"), t("Responsable y fecha de revisión", "Owner and review date")],
      prompt: t(`Completa esta frase: «Aprobaría el piloto de ${p.name.es} solo si ___; lo detendría si ___; revisaría el resultado el ___».`, `Complete this sentence: ‘I would approve the ${p.name.en} pilot only if ___; I would stop it if ___; I would review the result on ___.’`),
    },
  ];
}
  ...
  return [
    {
      ...
      takeaways: [t("Qué trabajo protegemos", "Which job we protect"), t("Qué vamos a medir", "What we will measure"), t("Qué opción elegimos", "Which option we choose"), t("Cómo comprobamos el resultado", "How we check the result")],
    },
    {
    ...
    },
    {
      ...
      title: t("Cómo tomaríamos la decisión en la práctica", "How we would make the decision in practice"),
      body: [t("Primero se descartan las opciones que no cumplen seguridad, requisitos técnicos o capacidad mínima. Una cifra ambiental favorable nunca compensa el incumplimiento de una barrera.", "First, options that fail safety, technical requirements or minimum capability are removed. A favourable environmental figure never compensates for a failed gate."), t(`Después se afronta esta dificultad concreta: ${p.tradeoff.es}.`, `The following specific difficulty is then addressed: ${p.tradeoff.en}.`), t(`La medida se prueba de forma controlada: ${p.intervention.es}. El resultado se comprueba mediante ${p.verification.es}.`, `The measure is tested under control: ${p.intervention.en}. The result is checked through ${p.verification.en}.`)],
